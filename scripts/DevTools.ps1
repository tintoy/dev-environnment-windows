<#
  .SYNOPSIS Environment script for development tools. Currently a little clumsy, but we'll tidy this up as we go.
  .PARAMETER DotNet
	Add .NET development tools to the environment.
  .PARAMETER CPP
	Add C++ development tools to the environment.
  .PARAMETER Jdk7
	Add Java 7 development tools to the environment.
  .PARAMETER Jdk8
	Add Java 8 development tools to the environment.
#>
...
<#
	.SYNOPSIS Convert a semicolon-delimited string to a sorted set of paths.
	.PARAMETER
...
<#
	.SYNOPSIS Convert a sorted set of paths to a semicolon-delimited string.
	.PARAMETER
		The path string to convert.
#>
...
If ($JdkVersion) {
	$env:JAVA_HOME = Join-Path ${env:ProgramFiles(x86)} "Java\jdk$JdkVersion"
	$env:JDK_HOME = Join-Path ${env:ProgramFiles(x86)} "Java\jdk$JdkVersion"
	$paths.Add(
	...
	$paths.Add(
		('C:\Development\Tools\activator\bin')
	) | Out-Null
}
...
# Configure the window / tab title
$windowTitle = "Dev"
